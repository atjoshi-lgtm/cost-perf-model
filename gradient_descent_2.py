...
def plot_performance_vs_hitrate(metro):
	...
	plt.legend()
	plt.grid()
	plt.savefig(f"served_from/{metro}_performance_vs_hitrate.png")
	plt.clf()
	

# I want to plot performance vs cache size for each metro. The x-axis should be the cache size in TB and the y-axis should be the P50 and P95 latencies in ms. The function should be called plot_performance_vs_cache_size and should take the metro name as an argument.
def plot_performance_vs_cache_size(metro):
	cache_sizes = []
	p50s = []
	p95s = []
	for hitrate in range(25, 1001, 25):
		TRY_HITRATES[metro] = hitrate / 10.0
		p50, p95 = compute_performance_localized(metro)
		cache_size = FDS[metro].nearest_cache_for_hitrate(hitrate / 10.0)
	...
	plt.figure(figsize=(10, 6))
	plt.plot(cache_sizes, p50s, label='P50 Latency (ms)')
	plt.plot(cache_sizes, p95s, label='P95 Latency (ms)')
	plt.xlabel('Cache Size (TB)')
	plt.ylabel('Latency (ms)')
	plt.title(f'Performance vs Cache Size for Metro: {metro}')
	plt.legend()
	plt.grid()
	plt.savefig(f"served_from/{metro}_performance_vs_cache_size.png")
	plt.clf()


for metro in metros:
	#plot_performance_vs_hitrate(metro)
	plot_performance_vs_cache_size(metro)
